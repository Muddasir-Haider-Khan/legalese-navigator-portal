...
const Hero = () => {
  ...
  return (
    <section className="bg-gradient-to-br from-rocket-blue-500 to-rocket-blue-700 text-white relative min-h-[90vh] flex items-center transition-all duration-500">
      ...
      <div className={`absolute top-40 right-20 w-40 h-40 rounded-full bg-rocket-blue-200/10 blur-2xl animate-float will-change-transform transition-opacity duration-1000 ${isLoaded ? 'opacity-100' : 'opacity-0'}`} style={{ animationDelay: "1.5s" }}></div>
      <div className={`absolute bottom-40 left-20 w-40 h-40 rounded-full bg-rocket-blue-200/10 blur-2xl animate-float will-change-transform transition-opacity duration-1000 ${isLoaded ? 'opacity-100' : 'opacity-0'}`} style={{ animationDelay: "2s" }}></div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          ...
          <div className={`hidden lg:block glass-card backdrop-blur-lg bg-white/10 rounded-xl p-8 border border-white/20 shadow-2xl transition-all duration-1000 transform ${isLoaded ? 'translate-x-0 opacity-100' : 'translate-x-20 opacity-0'} hover:shadow-glow hover:bg-white/15 transition-all duration-500`}>
            <div className="space-y-8">
              ...
              <Link to="/services" className="flex items-center gap-2 text-white hover:text-rocket-gray-200 transition-colors duration-300 group">
